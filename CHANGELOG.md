...
เคยมี routine ตรวจสอบอัตโนมัติทุกเช้าวันจันทร์ แต่ถอดออกแล้วเมื่อ 19 ส.ค. 2026 (ดูรอบล่าสุดด้านล่าง)
ตั้งแต่นี้การอัปเดตเนื้อหาเป็นการทำด้วยมือ — ถ้าจะหยิบ deck ไปใช้ ควรเทียบกับต้นทางก่อน
...
## 2026-08-19 — ถอดระบบตรวจสอบอัตโนมัติออก

**ลบ**

- ถอด scheduled job ที่ตรวจสอบเนื้อหาทุกเช้าวันจันทร์ออกทั้งหมด — deck ไม่ได้ใช้งานต่อแล้ว
  ลบ `scripts/` (prompt, ตัวรัน, launchd plist, ตัวติดตั้ง), ถอน launchd agent ออกจากเครื่อง
  และปิด cloud routine ฝั่ง claude.ai
- ตัดหัวข้อ "การอัปเดตอัตโนมัติ" ออกจาก README เพราะไม่เป็นความจริงอีกต่อไป

**เปลี่ยน**

- [CLAUDE.md](CLAUDE.md) เปลี่ยนหัวข้อ Weekly content refresh เป็นบันทึกว่าไฟล์ไหนเขียนข้อเท็จจริงซ้ำกับสไลด์
  และจะเก่าไปพร้อมกัน — เรื่องนี้ยังสำคัญแม้ไม่มีระบบอัตโนมัติ
- README ระบุชัดว่าถ้าจะหยิบ deck กลับมาใช้ ต้องไล่เช็คลิงก์และข้อเท็จจริงกับต้นทางก่อนเอง

**หมายเหตุ** เนื้อหาที่รอบตรวจ 19 ส.ค. พบว่าต้นทางเปลี่ยนไปอีก 4 จุด (Cowork availability,
Linux Desktop ไม่อัปเดตตัวเอง, Chrome protected actions, artifact บน Claude Code) **ยังไม่ได้นำมาใส่**


## 2026-08-19 — ย้ายรอบตรวจรายสัปดาห์มารันบนเครื่อง

**เปลี่ยน**

- รอบตรวจรายสัปดาห์ย้ายจาก cloud routine มารันบนเครื่อง Mac ของผู้ดูแลผ่าน `launchd`
  เพราะ GitHub connection ฝั่ง cloud เป็น read-only จึง push ไม่ได้ 2 รอบติด (17 และ 19 ส.ค.)
  ส่วน `gh` ในเครื่องมีสิทธิ์ครบอยู่แล้ว

**เพิ่ม**

- `scripts/weekly-audit-prompt.md` — คำสั่งของรอบตรวจ เก็บเป็นไฟล์ในโปรเจกต์เพื่อให้เห็นประวัติการแก้
- `scripts/run-weekly-audit.sh` — ตัวรัน มี preflight (เช็ค `gh`, เช็คว่าเข้า support.claude.com ได้)
  ทำงานใน clone แยกที่ `~/.local/state/` กัน lock ซ้อน และเก็บ log 12 สัปดาห์
- `scripts/install-weekly-audit.sh` + plist — ตั้งเวลาจันทร์ 09:00 และถ้าเครื่องหลับอยู่จะรันให้ตอนเปิดเครื่อง
...
- routine ได้คำเตือนเรื่องการอ่าน collection index เพิ่ม: หน้า collection มี navigation ของทั้งเว็บติดมาด้วย
  (grep ตรง ๆ จะได้ ~346 ลิงก์เท่ากันทุก collection ซึ่งไม่ใช่ของ collection นั้น) และหลาย collection
  เก็บบทความไว้ใน sub-collection ต้องไล่เข้าไปอ่านด้วย ไม่งั้นจะสรุปว่า "ไม่มีบทความใหม่" ทั้งที่ยังไม่ได้อ่าน
